/*
  Vendored from the Rare UI registry (https://www.rareui.com/r/code-block.json)
  with two local changes: the frame colours read the ecosystem contract tokens
  instead of the library's own neutrals, and the default accent is this site's
  signal. Re-adding this component with --overwrite will drop both.
*/
'use client'
...
function buildTheme(accent: string, mode: 'dark' | 'light' = 'dark') {
    ...
    const colors = dark
        ? {
              accent: accentTone,
              // Chrome comes from the ecosystem contract so the block sits in
              // the page rather than looking like a component from another site.
              bg: 'var(--surface-1)',
              border: 'var(--line-0)',
              ...
              muted: 'var(--ink-2)',
              gutter: 'var(--ink-3)',
              hoverWash: 'var(--surface-3)',
              floatBg: 'var(--surface-2)',
